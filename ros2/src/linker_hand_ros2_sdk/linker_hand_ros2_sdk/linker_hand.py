...
class LinkerHand(Node):
    ...
    def hand_setting_cb(self,msg):
        ...
        target_hand_type = params.get("hand_type")
        if target_hand_type != self.hand_type:
            return
        print(f"Received setting command: {data['setting_cmd']} for {self.hand_type}",flush=True)
        try:
            hand = self.api
            hand_left = self.hand_type == "left"
            hand_right = self.hand_type == "right"
            self.cmd_lock = True
            # Set maximum torque
            if data["setting_cmd"] == "set_max_torque_limits": # Set maximum torque
                torque = list(params["torque"])
                hand.set_torque(torque=torque)
                
            if data["setting_cmd"] == "set_speed": # Set speed
                if isinstance(params["speed"], list) == True:
                    speed = params["speed"]
            ...
            if data["setting_cmd"] == "set_electric_current": # Set current
                if isinstance(params["current"], list) == True:
                    hand.set_current(params["current"])
